...
for n_terms in terms_list:
    ...
    print(f"[n_terms={n_terms}] Clean fit std dev: {np.std(residuals_clean)}")
...
plt.plot(terms_list, stds, 'o-', label='Residual std dev')
...
plt.errorbar(terms_list, scale_factors, yerr=scale_factor_errors, fmt='o-', label='Perturbation scale factor', capsize=5)
plt.xlabel("Number of polynomial terms")
plt.ylabel("Perturbation scale factor")
plt.title("Perturbation Scale Factor vs. Number of Polynomial Terms")
plt.ylim(1 - 2 * np.std(scale_factors), 1 + 2 * np.std(scale_factors))
plt.legend()
plt.grid(True)
