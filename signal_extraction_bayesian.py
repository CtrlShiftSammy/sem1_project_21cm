...
yerr = sigma_arr
print(f"Mean sigma: {np.mean(sigma_arr)}, Median sigma: {np.median(sigma_arr)}, Max sigma: {np.max(sigma_arr)}, Min sigma: {np.min(sigma_arr)}")
# yerr = np.array([1.0] * len(sigma_arr))
...
scale_factors = []
scale_factor_errors = []
noise_floor_highest_prob = []
noise_floor_mean_params = []
noise_floor_mean_residuals = []
...
for n_terms in terms_list:
    ...
    print("*************************************************************************************")
    print()
    print("Fitting with n_terms =", n_terms)
    print()
    print("*************************************************************************************")
    print()
    ...
    stds_highest_prob.append(std_total)

    print(f"Standard deviation of residuals (best fit): {std_total}")
    print(f"Maximum abs residual (best fit): {np.max(np.abs(residuals_total))}")
    ...
    std_total = np.std(residuals_total)
    stds_mean_params.append(std_total)


    # noise_floor_mean_params.append(np.sum(residuals_total / (yerr ** 2)) / np.sum(yerr ** -2))
    noise_floor_numerator = 0
    noise_floor_denominator = 0
    for i in range(len(residuals_total)):
        numerator = (residuals_total[i] / yerr[i]) ** 2
    ...
    stds_mean_residuals.append(np.mean(residuals_subset))
...
plt.plot(terms_list, stds_highest_prob, color="#0047AB", marker='o', label='Residual std dev')
plt.plot(terms_list, noise_floor_highest_prob, color="#FF5733", marker='o', label='Noise floor')
...
plt.grid(True)
plt.title("Residual Standard Deviation vs. Number of Polynomial Terms (Highest Probability Params)")
plt.legend()
plt.savefig("residuals_std_vs_poly_terms_highest_prob.png", dpi=900)
print("Residuals standard deviation vs. polynomial terms plot saved.")
# plt.show()
plt.close()
plt.figure(figsize=(10, 6))
plt.plot(terms_list, stds_mean_params, color="#0047AB", marker='o', label='Residual std dev')
plt.plot(terms_list, noise_floor_mean_params, color="#FF5733", marker='o', label='Noise floor')
plt.yscale('log')  # use base-10 log scale on y-axis
plt.xlabel("Number of polynomial terms")
plt.ylabel("Std. dev. of residuals")
plt.axhline(0.001, color='r', linestyle='--', label='Target threshold (1 mK)')
plt.grid(True)
plt.title("Residual Standard Deviation vs. Number of Polynomial Terms (Mean Parameters)")
plt.legend()
plt.savefig("residuals_std_vs_poly_terms_mean_params.png", dpi=900)
print("Residuals standard deviation vs. polynomial terms plot saved.")
# plt.show()
plt.close()
plt.figure(figsize=(10, 6))
plt.plot(terms_list, stds_mean_residuals, color="#0047AB", marker='o', label='Residual std dev')
plt.yscale('log')  # use base-10 log scale on y-axis
plt.xlabel("Number of polynomial terms")
plt.ylabel("Std. dev. of residuals")
plt.axhline(0.001, color='r', linestyle='--', label='Target threshold (1 mK)')
plt.grid(True)
plt.title("Residual Standard Deviation vs. Number of Polynomial Terms (Mean of Residuals)")
plt.legend()
plt.savefig("residuals_std_vs_poly_terms_mean_residuals.png", dpi=900)
print("Residuals standard deviation vs. polynomial terms plot saved.")
# plt.show()
plt.close()
